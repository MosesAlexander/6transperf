#!/bin/bash
#Parameters
impl="n011-10G-n012-IP4-routing" # name of the tested implementation (used for logging)
dir="b" # valid values: b,f,r; b: bidirectional, f: forward (Left to Right, 6 --> 4), r: reverse (Right to Left, 6 <-- 4) 
max=4000000 # maximum packet rate
fs=64 # IPv6 frame size; IPv4 frame size is always 20 bytes less 
...
do
	...
	do 
		...
		echo "Command line is: ./x86_64-native-linuxapp-gcc/main --master-lcore 8 -l 0-3,8 -n1 -- --num-ports 2 --port0-lcoremask 0xc --port1-lcoremask 0x03 --num-queues 1 --mode dslite --dslite-testmode b4 --fps $r --packet-len $fs --duration $xpts --rxdelay 15 --txdelay 15"
		# Execute the test program
		sret=255
		while [[ "$sret" == "255" ]]; do
			./x86_64-native-linuxapp-gcc/main --master-lcore 8 -l 0-3,8 -n1 -- --num-ports 2 --port0-lcoremask 0xc --port1-lcoremask 0x03 --num-queues 1 --mode dslite --dslite-testmode b4 --fps $r --packet-len $fs --duration $xpts --rxdelay 15 --txdelay 15 > temp.out 2>&1
