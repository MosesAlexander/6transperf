sudo qemu-system-x86_64 -device virtio-net-pci,netdev=net0,mac=52:54:01:22:34:39 -netdev tap,id=net0,ifname=tap0,script=no,downscript=no \
			...
			-append 'root=/dev/sdb rw highres=off  console=ttyS0 hugepages=1536 ip=192.168.7.3::192.168.7.1:255.255.255.0::eth0 vga=0 uvesafb.mode_option=640x480-32 oprofile.timer=1 uvesafb.task_timeout=-1 isolcpus=1,2,3,4 nohz_full=1,2,3,4' \
			-machine q35 \
			...
			-device virtio-net-pci,netdev=net1,mac=52:54:01:32:34:02,bus=upstream1 -netdev tap,id=net1,ifname=tap2,script=no,downscript=no \
			-device virtio-net-pci,netdev=net2,mac=52:54:09:62:34:01,bus=upstream2 -netdev tap,id=net2,ifname=tap1,script=no,downscript=no \
			-gdb tcp::9000
